            ...
            try
            {
               ...
               // string companyName;
                ...
                if (loadfirsthtml.DocumentNode.InnerHtml.Contains("bseid"))
                {
                    ...
                    //companyName = loadfirsthtml.GetElementbyId("stick_stkexchg").GetAttributeValue("value", "");
                   // HtmlNode[] todayCurrentPrice_Array = loadfirsthtml.DocumentNode.SelectNodes("//div[@class='stkdigit']").ToArray();
                    //HtmlNode[] todayCurrentPrice_Array = loadfirsthtml.GetElementbyId("sp_val");.ToArray();
                    //outRefParams.stockName = companyName.ToString();
                    //todayCurrentPrice = float.Parse(todayCurrentPrice_Array[0].InnerText, CultureInfo.InvariantCulture.NumberFormat);
                    todayCurrentPrice = float.Parse(loadfirsthtml.GetElementbyId("sp_val").InnerHtml, CultureInfo.InvariantCulture.NumberFormat);
                    todayStatusPrice_Array = loadfirsthtml.DocumentNode.SelectNodes("//div[@id='sp_ch_prch']").ToArray();
                    todayStatusPrice = todayStatusPrice_Array[0].InnerText.ToString();
                }
